...
# add note DesignReview -> judge the posiblity of modality and language
class DesignReview(Phase):
    def __init__(self, chat_chain_instance = None, **kwargs):
        super().__init__(**kwargs)
        self.chat_chain_instance = chat_chain_instance
        
    def update_phase_env(self, chat_env):
        if chat_env.env_dict['modality'] == '' or chat_env.env_dict['language'] == '':
            chat_env.env_dict['break_flag'] = True
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        review = str(self.seminar_conclusion)
        value = review.split(',')[0].split()[-1]
        if value == "Yes":
            chat_env.env_dict['review'] = True
        elif value == "No":
            chat_env.env_dict['review'] = False
        else:
            raise TypeError("DesignReview_error:  <---No Valid Type---> ")
         
        if chat_env.env_dict['review']:
            log_visualize(
                "**[Software Info]**:\n\n {}".format("<  Pass  DesignReview!  >")
            )
        
        else:
            log_visualize(
                "**[Software Info]**\n\n {}".format("< Ready to redesign! >")
            )
            
            chat_env.env_dict['modality'] = ''
            self.chat_chain_instance.add_phase(current_phase="DesignReview", next_phase="DemandAnalysis")
            chat_env.env_dict['language'] = ''
            self.chat_chain_instance.add_phase(current_phase="DemandAnalysis", next_phase="LanguageChoose")
            chat_env.env_dict['review'] = True
            self.chat_chain_instance.add_phase(current_phase="LanguageChoose", next_phase="DesignReview")
...
class Annotation(Phase):
    ...
    def update_phase_env(self, chat_env):
        ...
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               ...
                               "annotation_files": ""})
        annotation_files = ""
        all_files = os.listdir(chat_env.env_dict['directory'])
        annotation_list = [file for file in all_files if file.endswith('.py')]
        annotation_files = ', '.join(annotation_list)
        self.phase_env['annotation_files'] = annotation_files 
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        chat_env.rewrite_codes('Finished adding annotations')
        log_visualize(
            "**[Software Info]**:\n\n {}".format(get_info(chat_env.env_dict['directory'], self.log_filepath)))
        return chat_env
...
# add note ------CodeConclusion
class CodeConclusion(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        chat_env.env_dict['code_conclusions'] = self.seminar_conclusion
        return chat_env
...
# add note ----- CommentJudgement
class CommentJudgement(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        
    def update_phase_env(self, chat_env):
        if chat_env.env_dict['review_comments'] == '' or chat_env.get_codes() == '':
            chat_env.env_dict['break_flag'] = True
        ...
        self.phase_env.update(
                        ...
                        "language": chat_env.env_dict['language'],
                        "codes": chat_env.get_codes(),
                        "conclusions": chat_env.env_dict['code_conclusions'],
                        "comments": chat_env.env_dict['review_comments']})
    
    def update_chat_env(self, chat_env) -> ChatEnv:
        value = str(self.seminar_conclusion)
        
        # keyword = value.split(',')[0].split()[-1]
        # if keyword == "Yes":
        #     chat_env.env_dict['judgement'] = True
        #     log_visualize(
        #         "**[Software Info]**:\n\n {}".format("< Pass the judgement ! >")
        #     )
        # elif keyword == "No":
        #     chat_env.env_dic['judgement'] = False
        #     log_visualize(
        #         "**[Software Info]:\n\n {}".format("< Fail to pass the judgement ! >")
        #     )            
        # else:
        #      raise TypeError("CommentJudgement_error:  <---No Valid Type--->")
        chat_env.env_dict['judgement'] = True
        log_visualize(
                "**[Software Info]**:\n\n {}".format("< Pass the judgement ! >")
            )
...
class CodeReviewModification(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        if chat_env.env_dict['judgement'] == True:
            ...
            log_visualize(
                "**[Software Info]**:\n\n {}".format("<  Have NO CodeReviewModification!  >")
            )
...
class TestErrorSummary(Phase):
    ...
    def update_phase_env(self, chat_env):
        ...
        chat_env.env_dict['test_error_summary'] = True
...
class TestModification(Phase):
    ...
    def update_phase_env(self, chat_env):
        print(chat_env.env_dict)
        
        if chat_env.get_codes() == '' or chat_env.env_dict['test_error_summary'] == False:
            chat_env.env_dict['break_flag'] = True
        ...
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               ...
                               "exist_bugs_flag": True, #default set to avoid KeyError
                               "codes": chat_env.get_codes()
                               })
...
class EnvironmentDoc(Phase):
    ...
    def update_phase_env(self, chat_env):
        if chat_env.env_dict['modality'] == '' or chat_env.env_dict['language'] == '' or chat_env.get_codes() == '' :
            chat_env.env_dict['break_flag'] = True
...
class Manual(Phase):
    ...
    def update_phase_env(self, chat_env):
        if chat_env.get_codes() == '' or chat_env.get_requirements() == '':
            chat_env.env_dict['break_flag'] = True
